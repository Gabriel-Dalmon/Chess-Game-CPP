...
//------------------------------------------------------------------------------
MovableType Pawn::IsMovable(const Board& board, int start_position[2], 
	int destination_position[2]) const
{
	...
	const int destination_index = 
		Helper::GetSquareIndex(destination_position, BOARD_WIDTH);
	...
	/** CAPTURE MOVE CHECK */
		...
		/** EN PASSANT CAPTURE MOVE CHECK */
			...
			const bool is_en_passant_movable = IsEnPassantMovable(grid, 
				start_column, start_row, destination_column);
...
//------------------------------------------------------------------------------
inline bool Pawn::IsVerticalMovable(const APiece* const grid[BOARD_SIZE], 
	int start_column, int start_row, int destination_row, 
	int destination_index) const
{
	...
	const bool is_normal_move_square_occupied = 
		grid[normal_move_index] != nullptr;
...
*/
inline bool Pawn::IsCaptureMovable(const APiece& target_piece, int start_column,
	int start_row, int destination_column, int destination_row) const
{
	//-------------------------------------------------------------------------
	...
	const bool capture_horizontal_constraint = 
		start_column + 1 == destination_column ||
		start_column - 1 == destination_column;
...
inline bool Pawn::IsEnPassantMovable(const APiece* const grid[BOARD_SIZE],
	int start_column, int start_row, int destination_column) const
{
	//-------------------------------------------------------------------------
	/** Check en passant move pattern. */
	const bool en_passant_row_constraint = 
		start_row == (m_direction == EPawn::Direction::DOWN) *
		BOARD_HEIGHT - m_direction * 4;
	if (en_passant_row_constraint == false) return false;

	const bool en_passant_horizontal_constraint = 
		start_column + 1 == destination_column || 
		start_column - 1 == destination_column;
	...
	const APiece* p_side_square = 
		grid[destination_column + start_row * BOARD_WIDTH];
